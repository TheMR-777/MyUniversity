...
struct student: internals::persona
{
	...
	Q_OBJECT 
public slots:
	void respond(my_data_type msg) const { fmt::print("[ {:10} ] - Sir, Yes Sir! Responding to: [ {} ]\n", my_name, msg); }
};
...
struct partner: internals::persona
{
	...
	Q_OBJECT 
public slots:
	void respond(my_data_type msg) const { fmt::print("[ {:10} ] - Respected Sir, we are available. Responding to: [ {} ]\n", my_name, msg); }
};