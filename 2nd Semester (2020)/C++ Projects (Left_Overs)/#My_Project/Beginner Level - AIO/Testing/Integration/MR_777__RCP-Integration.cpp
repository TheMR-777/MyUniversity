...
namespace External
{
	void Write(const vector<Subject>& sbj, string filename)
	{
	...
	void Subject_wise(vector<Profile> &students)
	{
		...
		for (int i = 0; i < students[0].subjects.size(); i++)
		{
			...
			for (int j = 0; j < students.size(); j++)
			{
				...
				temp_f = class_function::PT_AGE(sub_ref->o_marks, sub_ref->t_marks);
...
int main()
{
	...
	// Decision Making & Redirection
		...
		else
		{
			...
			else if (s_choice == "MR_777" || s_choice == "mr_777" || s_choice == "MR" || s_choice == "mr" || s_choice == "mr777")
			{
				Secrets::SecretMSG();
			}
			...
			else if (s_choice == "Elsa Grey" || s_choice == "ElsaGrey" || s_choice == "elsa grey" || s_choice == "elsagrey" || s_choice == "Elsa grey" || s_choice == "Elsagrey" || s_choice == "EG" || s_choice == "eg")
			{
